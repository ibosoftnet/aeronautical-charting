...
- **`ad_hp_runways`** (aspatial): RWY-DIR (pist yönü: bearing) + RWY-INFO (pist çifti: uzunluk/genişlik) verisi, birbirine eşleştirilmiş.
...
| `ad_hp_runways` | aspatial | yok (koordinat kaynakta yok) | RWY-DIR (yön) + RWY-INFO (çift) join | 1 satır = 1 pist YÖNÜ | ~55.000 |
...
| `info_source_file` | TEXT | — | RWY-INFO XML dosya adı |
...
- **Eksik boyut verisi**: `info_joined=1` olsa da `info_length`/`info_width`/`info_dim_unit` NULL olabilir — kaynakta gerçekten yok (örn. bazı heliport-stili designator'larda).
...
- **REAL**: koordinatlar, bearing (`true_bearing`/`mag_bearing`), pist boyutları (`info_length`/`info_width`).
- **INTEGER**: join/eşleşme bayrakları (`usage_joined`, `info_joined`), rastgele üretilen `match_id`.
- **TEXT**: kodlar, designator'lar, tarihler (ham `DD/MM/YYYY` string), kurum adları, birim etiketleri, `mid` değerleri (büyük sayılar olabileceği için int'e çevrilmez).
...
- `tailored-data.jsonc` — manuel override/ek kayıt dosyası (bkz. dosya içi yorumlar).
...
- `rwy-info-{afr,am,asi-aus,eur}.xml` — pist ÇİFT verisi (uzunluk/genişlik), aynı `am`→`am-pac` eşlemesiyle.
